								...
								angular.forEach(scope.temp_employees, function (ctr_value, ctr_key) {
									...
									if ((scope.temp_employees.length - 1) == ctr_key) {
										angular.forEach(scope.temp_employees, function (value, key) {
											...
											var html_tr = '<tr class="dependent-hover-container ' + scope.isTrError + ' "><td><input type="checkbox" ng-model="temp_employees[' + key + '].checkboxSelected" ng-click="empCheckBoxClicked(' + key + ')"></td><td><span class="icon"><i class="fa fa-check" style="display: none;"></i><i class="fa fa-times" style="display: none;"></i><i class="fa fa-circle-o-notch fa-spin" style="display: none;"></i></span><span class="fname">' + value.employee.fullname + '</span><button class="dependent-hover-btn" ng-click="openEditDetailsModal(' + key + ')">Edit</button></td><td>' + value.employee.dob + '</td><td>' + value.employee.email + '</td><td>+' + value.employee.mobile_area_code + '</td><td>' + value.employee.mobile + '</td><td ng-if="spending_account_status.medical">' + parseFloat(value.employee.credits).toLocaleString('en', option) + '</td><td ng-if="'+ hasMedicalBalance +'">' + value.employee.medical_balance_entitlement.toLocaleString('en', option) + '</td><td ng-if="spending_account_status.wellness">' + parseFloat(value.employee.wellness_credits).toLocaleString('en', option) + '</td><td ng-if="'+ hasWellnessBalance +'">' + value.employee.wellness_balance_entitlement.toLocaleString('en', option) + '</td>';
											var emp_ctr = 0;
