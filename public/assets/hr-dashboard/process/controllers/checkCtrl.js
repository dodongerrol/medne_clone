...
checkCtrl.controller('checkCtrls', function( $scope, $http, $stateParams, $state, hrSettings ){
	...
	vm._getLinkedAccountsWithCurrent_	=	function(){
		var url = window.location.origin + `/hr/get/corporate_linked_account?limit=99&page=1&total_enrolled_employee_status=true&total_enrolled_dependent_status=true&except_current=enable`;
		$http.get(url)
		.success(function(response){
			console.log(response);
			vm.allLinkedAccountsArr = response.data;
			vm.linkedAccountsPagi = response;
		});
	}
