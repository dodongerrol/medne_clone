app.directive('healthSpendingAccountSummaryDirective', [
	...
	'$timeout',
	'employeeFactory',
	function directive( $state, removeEmployeeFactory, dependentsSettings, $timeout, employeeFactory ) {
		return {
			restrict: "A",
			scope: true,
			link: function link( scope, element, attributeSet ) {
				console.log( 'healthSpendingAccountSummaryDirective running!' );
				scope.selectedEmployee =	employeeFactory.getEmployeeDetails();
				...
				if(!scope.deleted_emp_details && !scope.emp_details){
					$timeout(() => {
            ...
            .then(function (response) {
              ...
              scope.hideLoading();
				...
				scope.nextBtn	=	function(){
					scope.showLoading();
					$state.go('member-remove.health-spending-account-confirm');
				}
