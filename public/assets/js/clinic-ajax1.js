...
// window.base_url = 'https://localhost/medicloud_web/public/app/'; 
    ...
    jQuery("#save-doctordetails").click(function(e){
        e.preventDefault();
         if(jQuery("#form-doctor").valid() ==true){
            ...
            var dataValues = 'clinicid='+clinicid+'&name='+name+'&qualification='+qualification+'&speciality='+speciality+'&mobile='+mobile+'&phone='+phone+'&email='+email;
            jQuery.blockUI({ message: '<h1> '+base_loading_image+' <br /> Please wait for a moment</h1>' });
            jQuery.ajax({
                type: "POST",
                ...
                success : function(data){
                    if(data!=0){
                        ...
                        jQuery("#ajax-error-site").html('Doctor already exist');
                    }
                    jQuery.unblockUI();
