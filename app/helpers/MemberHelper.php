	...
	public static function getMemberCreditReset($member_id, $term, $spending_type)
	{
		...
		if($term == "current_term") {
			...
			if($credit_resets) {
				...
				return ['start' => date('Y-m-d', strtotime($first_plan)), 'end' => $today, 'id' => null];
			}
		} else {
			$credit_resets = DB::table('credit_reset')
								->where('id', $member_id)->where('user_type', 'employee')
	...
	public static function checkMemberAccessTransactionStatus($member_id)
	{
		...
		if($status) {
			...
			if($spending['medical_enabled'] == true && $spending['medical_reimbursement'] == false) {
				if($spending['medical_enabled'] == true && $spending['wellness_enabled'] == true && $spending['medical_reimbursement'] == true) {
					return false;
				}
