<!-- {{ HTML::script('assets/dashboard/calender.js') }} -->
  
  
<script type="text/javascript" src="<?php echo $server; ?>/assets/dashboard/calender.js?_={{ $date->format('U') }}"></script>
      ...
      <div id="content-two" hidden>
			...
			<div class="form-group">
			    ...
			    <div class="col-sm-2" style="padding-right: 0; margin-left: 50px;">
					...
					<input type="text" id="block-time-Duration-search" class="tf form-control" min="0" value="0">
			    </div>
...
<input type="hidden" id="appCount" value="{{$appCount}}">
...
<a href="">
	<div id="new_Appointment_notification">
		<span id="notify_text"></span>
	</div>
</a>
	
<div id="byTabs" hidden>
	<ul class="nav nav-tabs calendarViewTabs doctor-list" role="tablist">
	    <!-- <li role="presentation" class="active"><a href="#home" aria-controls="home" role="tab" data-toggle="tab">Home</a></li> -->

	    <?php 
	    	foreach ($doctorlist as $val) { 
	    ?>
	            <li role="presentation" class="{{ $val->DoctorID}}">
	            	<a id="{{ $val->DoctorID}}" href="#{{ $val->DocName}}" aria-controls="{{ $val->DocName}}" role="tab" data-toggle="tab">
	            		<img src="{{ URL::asset('assets/images/ico_Profile.svg') }}" alt=""> {{ $val->DocName}}
	            	</a>
	            </li>
         <?php 
         	} 
  ...
  <div class="form-group">
    ...
    <div class="col-sm-3">
    	...
    	<i id="ok-icon" class="glyphicon" style="color: #1b9bd7"></i>&nbsp;&nbsp;<span class="service-selection" id="">Select a service</span><div class="ext-right"><i class="glyphicon glyphicon-chevron-down"></i></div></label>
    		<ul class="dropdown-menu ul-width" id="service-list" style="width: 392px; max-height: 258px; overflow-y: auto; overflow-x: hidden;">
				...
				<li id="" style="padding: 5px 15px 5px 15px; color: #555555;" data-toggle="modal" data-target="#reserveModal" data-dismiss="modal">
    ...
    <div id="Cost-Time-duration">
	    <div class="form-group">
		    ...
		    <!-- </div> -->
		    <label class="col-sm-1 details-label" style="padding: 0px; padding-left: 33px;">Duration</label>
		    <input type="text" id="service-time-Duration" class="dropdown-btn col-sm-1" style="height: 15px; width: 55px; margin-left: 62px;">
    ...
    <div class="col-sm-8">
    	<input type="text" id="notes-single" class="dropdown-btn input-width" placeholder="Notes / Instructions" style="height: 27px;">
    </div>
    </div>
    ...
    <div class="panel panel-default" style="margin: 0px;">
		...
		<div class="col-xs-10" style="padding-left: 205px;"><button type="button" id="add-new-customer" class="btn btn-update font-type-Montserrat" style="height: 26px;"><i class="glyphicon glyphicon-plus" style="font-size: 11px;"></i> New Patient</button></div><br><br>
		</div>
		...
		<br>
		<div class="ext-right" style="padding-right: 15px;"><span id="close"><i class="glyphicon glyphicon-remove" style="color: #999999;"></i></span></div>
		<br>
		...
		<!-- Telephone -->
		    ...
		    <!-- <div class="col-sm-2">
		    	<input type="text" id="phone-code" class="dropdown-btn" placeholder="65" style="height: 35px; width: 75px;">
		    </div>
		    <div class="col-sm-5" style="padding: 0px; padding-left: 11px;">
		    	<input type="text" id="phone-no" class="dropdown-btn" placeholder="Phone" style="height: 30px; width: 235px;">
		    </div> -->
		    <div class="col-md-8">
		...
		<!-- City / State / Zip -->
		    ...
		    <div class="col-sm-8"><button type="button" id="save-appointment" class="btn btn-update font-type-Montserrat ext-left" hidden>Save Appointment</button></div>
		    <div class="col-sm-8"><button type="button" id="update-appointment" class="btn btn-update font-type-Montserrat ext-left hide">Update Appointment</button></div>
		    </div>
		    <br>
		...
		<div id="check-save" class="hide" style="width: 100%;overflow: hidden;text-align: left;">
			...
			<button type="button" id="save-appointment" class="btn btn-update font-type-Montserrat save-btn" style="float: none;">Save Appointment</button>
			</div>
		</div>
...
<div class="modal fade" id="reserveModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-8">
    	...
    	<div class="ext-right"><i class="glyphicon glyphicon-chevron-down"></i></div></label>
    ...
    <div class="col-sm-8">
    	...
    	<button type="button" class=" btn btn-update font-type-Montserrat col-sm-3" id="blocker-reserve">Reserve</button>
    </div>
    </div>
...
<div style="overflow: hidden;">
    ...
    <table class="appointment-det-container" style="font-size: 13px;">
  	...
  	<div class="appointment-details" style="padding-top: 10px; padding-bottom: 10px; text-align: center; background: white !important; clear: both;">
        	<button type="button" id="edit-appointment-details" class="edit-appointment appt-details-btn hide-buttons" style="font-size: small;">Edit Details </button>
        	...
        	<button type="button" id="concluded-appointment" class="appt-details-btn hide-buttons" style="font-size: small;">Claim</button>
    ...
    <div class="balance" hidden>
	    ...
	    <div class="col-md-5">
	    	<input type="number" class="form-control" name="bill_amount_single" id="bill_amount_single" placeholder="Please enter Total Bill" style="font-size: 12px;height: 30px !important;">
	    </div>
    ...
    <div class="balance-co-paid" hidden>
    	<div class="col-md-10">
    		<p style="font-size: 10px;color: #555555;">* Upon finishing the billing will conclude the appointment.</p>
	    	<!-- <h5 style="color: #555;font-weight: 700">Medi Credit: <span id="user-credit-balance"></span></h5> -->
	    </div>
	    ...
	    <div class="col-md-5">
	    	<input type="number" class="form-control" name="bill_amount_co_paid" id="bill_amount_co_paid" placeholder="Total Medication Bill" style="font-size: 12px;height: 30px !important;">
	    </div>
	    <div class="col-md-4" style="margin-bottom: 20px;padding-left: 0;width: 44.333333%;">
	    	<button id="calc-bill-co-paid" class="btn btn-primary" style="height: 29px;font-size: 12px;padding: 0px 12px;position: relative;top: -1px;">Calculate Bill</button>
	    	<button id="" class="btn btn-warning cancel1 calc-cancel" style="height: 29px;font-size: 12px;padding: 0px 12px;background-color: #f0ad4e;border-color: #eea236;">Cancel</button>
	    </div> 
    ...
    <div class="summary-receipt-co-paid" style="margin-left: 20px;" hidden>
    	<div class="col-md-11">
	    	<div class="white-space-20"></div>
	    </div>
	    ...
	    <div class="col-md-11 text-right" style="margin-bottom: 20px">
	    	<button id="" class="btn btn-warning cancel2 calc-cancel" style="height: 29px;font-size: 12px;padding: 0px 12px;background-color: #f0ad4e;border-color: #eea236;">Back</button>
	    	<button class="btn btn-success" style="height: 29px;font-size: 12px;padding: 0px 12px;position: relative;top: -1px;background-color: #5cb85c;border-color: #4cae4c;" id="finish-transaction-co-paid">Finish</button>
	    </div>
    </div>
...
<div id="error-messages" title="Error Message" style="display: none; font-family:'Open Sans', 'sans-serif'; background: #BCE5F7 !important;">
	<div id="cal-error" class="well well-lg" style="font-size: 14px; color: #C10E11; margin: 0px; text-align: center; background: white;">
...
<div class="modal fade" id="info-modal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog" style="width: 950px!important;" role="document">
    <div class="modal-content">
      <div class="modal-body no-padding">
       	<img src="{{ URL::asset('assets/pdf/Health Partner SOP-1.png') }}" style="width: 100%;" />
       	<img src="{{ URL::asset('assets/pdf/Health Partner SOP-2.png') }}" style="width: 100%;" />
       	<img src="{{ URL::asset('assets/pdf/Health Partner SOP-3.png') }}" style="width: 100%;" />
       	<img src="{{ URL::asset('assets/pdf/Health Partner SOP-4.png') }}" style="width: 100%;" />
      </div>
    </div>
  </div>
</div>
