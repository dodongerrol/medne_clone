    ...
    <div class="invoice-content">
      ...
      <div class="header">
        ...
        <div class="text-right item">
          <h1 style="font-size: 35px !important;color: #000 !important;font-family: 'Open Sans', sans-serif !important;margin: 0px;">INVOICE</h1>
      ...
      <div class="bill-to">
        ...
        <div class="item right-wrapper" >
          ...
          <p><label>Invoice Date: </label> {{date('F d, Y', strtotime($invoice_date))}}</p>
          <p><label>Payment Due: </label> {{date('F d, Y', strtotime($invoice_due))}}</p>
      ...
      <table class="table table-responsive text-center" style="border-bottom: 2px solid #DCDFE0;width: 100%;">
        ...
        <tr class="tbody">
          <td style="text-align: left !important;padding-left: 30px;">
            ...
            <p>Active Type ID: #{{ $customer_active_plan_id }}</p>
            <p>No. of employees: {{$number_employess}}</p>
            <p>Billing Frequency: Annual</p>
            <p>Start Date: {{date('F d, Y', strtotime($plan_start))}}</p>
            <p>End Date: {{date('F d, Y', strtotime($plan_end))}}</p>
        ...
        <tr class="tbody" style="border-top: 1px solid #eee;">
          <td style="text-align: left !important;padding-left: 30px;">
            <p>Active Plan Type: {{ $dependent['account_type'] }}</p>
            <p>No. of dependents: {{ $dependent['total_dependents'] }}</p>
            <p>Start Date: {{ $dependent['plan_start'] }}</p>
            <p>End Date: {{ $dependent['plan_end'] }}</p>
