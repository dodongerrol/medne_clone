		  ...
		  // validate all first
			...
			foreach ($temp_users as $key => $user) {
				...
				$temp_enrollment_data = array(
					...
					'cap_per_visit'			=> number_format($user['cap_per_visit']),
					'credits'				=> number_format(!isset($user['medical_credits'])) || number_format($user['medical_credits']) == null ? 0 : number_format($user['medical_credits']),
					'medical_balance_entitlement'				=> number_format(!isset($user['medical_credits'])) || number_format($user['medical_credits']) == null ? 0 : number_format($user['medical_credits']),
					'wellness_credits'		=> number_format(!isset($user['wellness_credits'])) || number_format($user['wellness_credits']) == null ? 0 : number_format($user['wellness_credits']),
					'wellness_balance_entitlement'				=> number_format(!isset($user['wellness_credits'])) || number_format($user['wellness_credits']) == null ? 0 : number_format($user['wellness_credits']),
					'postal_code'			=> null,
